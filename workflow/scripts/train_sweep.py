...
def train_test_under_agent(cfg: DictConfig, dataset: str, checkpoints_base_path: str):
    wandb.init(
        job_type="train_sweep",
...
def main(
    ...
    train_under_test_agent_ = partial(
        train_test_under_agent, cfg, dataset, checkpoints_base_path
    )
    wandb.agent(
        sweep_id,
        train_under_test_agent_,
        cfg.wandb.user,
        cfg.wandb.project,
        count=cfg.sweep.runs_per_job_count,
    )
