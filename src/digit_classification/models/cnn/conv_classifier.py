...
class ConvClassifier(L.LightningModule):
    ...
    def __init__(self, data_sample: Tuple[T.Tensor, T.Tensor], conv_blocks_1: int,
        ...
        assert digit_img_sample.shape[-1] == self.IMG_SIZE \
        ...
        self.network = nn.Sequential(
            nn.Conv2d(1, hidden_conv_channels, 3), nn.ReLU(),
            *[nn.Sequential(nn.Conv2d(hidden_conv_channels,
                    hidden_conv_channels, 3), nn.ReLU())
...
if __name__ == '__main__':
    ...
    class TestConvClassifier(unittest.TestCase):
        def test_network(self):
            ...
            model = ConvClassifier((labels[:1], digit_imgs[:1]), conv_blocks_1=2,
                    conv_blocks_2=2, hidden_conv_channels=8, mlp_depth=3,
        ...
        def test_training(self):
            ...
            train_loader_factory = partial(T.utils.data.DataLoader,
                    batch_size=32, num_workers=(multiprocessing.cpu_count() - 1),
                    pin_memory=True)
            datamod = LabelledDigitsModule(preproc, train_set, 'use_test',
                    train_loader_factory, train_loader_factory,
                    test_frac=0.1, val_frac=0.1)
            
            model = ConvClassifier(datamod.get_data_sample(), conv_blocks_1=2,
                    conv_blocks_2=2, hidden_conv_channels=8, mlp_depth=3,
                    optimizer_factory=partial(T.optim.Adam, lr=1.0e-3))
            ...
            labels_pred, labels_truth = \
                datamod.invert_setup_on_prediction(pred_batches)
