...
class PlotTarget:
    ...
    def get_figure(self, num_figures_per_column: int) -> plt.Figure:
        ...
        figsize = (
            plotsize[0] - 2 * self.figure_padding,
    ...
    def save_figure(self, fig: plt.Figure) -> None:
        self.fig_store.savefig(fig, bbox_inches="tight", pad_inches=self.figure_padding)
        plt.close(fig)
...
class Paper(PlotTarget):
    def __init__(
        self, fig_store: FigureStore, columnwidth_pt: float, fontsizes: Dict[str, float]
    ) -> None:
        super().__init__(fig_store)
...
class RocCurveTemplate:
    ...
    def plot(
        ...
        fig: plt.Figure,
        digit: int,
        labels_pred: List[NDArray],
        labels_truth: List[NDArray],
        pred_ids: List[str],
    ):
